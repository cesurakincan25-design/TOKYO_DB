...
/* ── HANAMI KOYU PEMBE TEMA ── */
body.theme-light #app-sidebar {
  background: linear-gradient(180deg, #220a18 0%, #1a0614 100%) !important;
  border-right-color: rgba(249,168,212,0.2) !important;
}
body.theme-light .nav-link { color: rgba(252,231,243,0.45) !important; }
body.theme-light .nav-link:hover, body.theme-light .nav-link.active { color: #f9a8d4 !important; }
body.theme-light #main-content { background: #1a0614 !important; }
body.theme-light .washi-panel {
  background: rgba(45,10,30,0.95) !important;
  border-color: rgba(249,168,212,0.25) !important;
}
body.theme-light .glass-panel {
  background: rgba(45,10,30,0.7) !important;
  border-color: rgba(249,168,212,0.15) !important;
}
body.theme-light .cyber-card, body.theme-light .tokyo-card {
  background: rgba(45,10,30,0.8) !important;
  border-color: rgba(249,168,212,0.15) !important;
}
body.theme-light .cyber-card:hover, body.theme-light .tokyo-card:hover {
  border-color: rgba(249,168,212,0.5) !important;
  box-shadow: 0 0 20px rgba(249,168,212,0.15) !important;
}
...
body.theme-light #tab-bar {
  background: #220a18 !important;
  border-color: rgba(249,168,212,0.2) !important;
}
body.theme-light .tab-item { color: rgba(252,231,243,0.4) !important; background: rgba(45,10,30,0.5) !important; }
body.theme-light .tab-item.active { color: #f9a8d4 !important; background: rgba(45,10,30,0.95) !important; border-bottom-color: #f9a8d4 !important; }
body.theme-light #ticker-wrap { background: #220a18 !important; border-color: rgba(249,168,212,0.2) !important; }
body.theme-light .petal { background: rgba(249,168,212,0.5) !important; }
body.theme-light .torii-divider { background: linear-gradient(90deg,transparent,#f9a8d4,#f0abfc,#f9a8d4,transparent) !important; }
/* Stat kartları */
body.theme-light .stat-card, body.theme-light [class*="stat"] {
  background: rgba(45,10,30,0.6) !important;
  border-color: rgba(249,168,212,0.2) !important;
}
/* Scrollbar */
body.theme-light ::-webkit-scrollbar-thumb { background: rgba(249,168,212,0.3) !important; }
...
/* Rez karakterler - biraz daha soluk */
#rez-character-grid .cyber-card { background: #160e2a !important; border-color:rgba(200,168,75,.15) !important; color:#e8e2d9; opacity:0.9; }
#rez-character-grid .cyber-card:hover { background:#221838 !important; opacity:1; }
/* Stat cards */
...
<div class="washi-panel p-4 border-l-4 cursor-pointer" onclick="UI.switchTab('cases')" style="border-left-color:var(--gold)"><div class="font-mono text-[10px] tracking-widest mb-1" style="color:var(--gold)">DAVA</div><div id="stat-cases" class="font-display font-black text-3xl" style="color:var(--ink)">0</div></div>
        ...
        <!-- ── REZ KARAKTERLER ── -->
        <div class="mt-10 mb-2" id="rez-chars-section">
          <div class="flex items-center gap-3 mb-4">
            <div class="h-px flex-1" style="background:linear-gradient(90deg,transparent,var(--sakura)40)"></div>
            <div class="flex items-center gap-2 font-mono text-[11px] tracking-[0.3em] uppercase" style="color:var(--sakura)">
              <i class="fas fa-archive"></i>
              REZ KARAKTERLER
              <span id="counter-rez-chars" style="color:var(--gold);opacity:.7"></span>
            </div>
            <div class="h-px flex-1" style="background:linear-gradient(90deg,var(--sakura)40,transparent)"></div>
          </div>
          <div id="rez-character-grid" class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 opacity-75"></div>
        </div>
      </div>
      <!-- Organizations -->
                    ...
                    <div id="admin-chars" class="admin-panel glass-panel p-6 cyber-border">
                        ...
                        <form id="char-form" onsubmit="Admin.saveCharacter(event); return false;" class="grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-xs">
                            ...
                            <div class="md:col-span-2">
                                <label class="text-cyber-blue block mb-1">Karakter Türü & Güvenlik</label>
                                <!-- Profil Tipi -->
                                <div class="mb-3">
                                  <label class="text-gray-400 text-[10px] uppercase tracking-widest block mb-1">Karakter Tipi</label>
                                  <select id="c-profile-type" class="w-full bg-black/60 border border-gray-700 text-white text-xs px-2 py-1.5 font-mono">
                                    <option value="main">Ana Karakter</option>
                                    <option value="profile">Rez Karakter</option>
                                  </select>
                                </div>
                                <!-- Classified -->
                                <label class="flex items-center gap-2 mb-2 cursor-pointer hover-sfx">
                                    <input type="checkbox" id="c-classified" class="cyber-checkbox click-sfx">
                                    <span class="text-red-400 font-bold uppercase tracking-widest text-[11px]">🔴 CLASSIFIED (Redacted — fotoğraf gizli)</span>
                                </label>
                                <!-- Locked -->
                                <label class="flex items-center gap-2 mb-2 cursor-pointer hover-sfx">
                                    <input type="checkbox" id="c-locked" class="cyber-checkbox click-sfx">
                                    <span class="text-yellow-400 font-bold uppercase tracking-widest text-[11px]">🔒 LOCKED (fotoğraf görünür, kilit ikonlu)</span>
                                </label>
                                <!-- Archived -->
                                <label class="flex items-center gap-2 cursor-pointer hover-sfx">
                                    <input type="checkbox" id="c-archived" class="cyber-checkbox click-sfx">
                                    <span class="text-gray-400 font-bold uppercase tracking-widest text-[11px]">📦 ARŞİVLENDİ (ana listeden gizle)</span>
                                </label>
                            </div>
                            ...
                            <div class="md:col-span-4">
                              <label class="text-cyber-blue block mb-1"><i class="fas fa-music mr-1"></i>Theme Song (Spotify veya YouTube linki)</label>
                              <input type="text" id="c-theme-song" placeholder="https://open.spotify.com/track/... veya https://youtu.be/...">
                              <p class="text-gray-600 text-[10px] mt-1">Karakter profili açılınca otomatik çalar</p>
                            </div>
...
const TOKYO_THEMES = {
  ...
  hanami: {
    '--sakura':'#f9a8d4', '--torii':'#db2777', '--gold':'#f0abfc',
    '--bamboo':'#fbcfe8', '--indigo':'#831843', '--ink':'#fce7f3',
    '--mist':'#2d0a1e',   '--paper':'#1a0614',  '--night':'#110310',
    '--bg-grid':'#1a0614',
    body: 'background: radial-gradient(ellipse at 30% 40%, #2d0a1e 0%, #1a0614 50%, #110310 100%)',
  },
  night: {
